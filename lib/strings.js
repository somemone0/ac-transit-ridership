...
export const strings = {
  ...
  story: {
    ...
    mapTwoLabel: "Map of inferred morning bus trips to and from UC Berkeley",
    ...
    one: {
      ...
      simulated:
        ...
        + "ridership estimates from AC Transit. Simulated rides make up about 40% of rides in 2019, "
        + "but decline to under 5% of rides after 2021. For some routes like the 1T in East Oakland "
        + "and new routes after the 2025 Realign changes, simulated rides make up the majority of "
        + "rides.",
      berkeley2020:
      ...
      pandemic:
        "During the pandemic, ridership sharply declined by 80% throughout the city. In stops near "
        + "UC Berkeley, ridership declined by as much as 97%.",
      sanPablo:
    ...
    two: {
      ...
      ipfAccuracy:
        ...
        + "which does collect these trips, this method achieved an 80% accuracy rate. The model is "
        + "unable to estimate basic elements of bus usage, like transfers between buses.",
      fromCampus:
        "These are estimated riders that travel from UC Berkeley in the morning on weekdays.",
      toCampus:
        "These are estimated riders that travel to UC Berkeley in the morning on weekdays. We can "
        + "infer that these are commuters.",
      marks:
        "Commuters to UC Berkeley are estimated to come from Rockridge BART and UC Village more "
        + "than other places.",
      acHome:
    ...
    three: {
      ...
      // Each {p..} is a speed in mph from numbers.json.
      pandemicTraffic:
        "Like ridership, traffic in Berkeley declines during the pandemic. The median bus speed in "
        + "Berkeley rose from {base50} mph to {dec50} mph, the slowest tenth of bus-km from {base10} "
        + "to {dec10}, and the fastest tenth from {base90} to {dec90}.",
      samePattern:
        "Traffic followed the same pattern as ridership, only increasing when UC Berkeley students "
        + "returned, and returning to normal when UC Berkeley fully ended pandemic measures in 2023.",
  ...
  dates: {
    ...
    // A closed range of months, e.g. "Feb. 2019-Jan. 2020".
    monthRange: "{from}\u2013{to}",
  },

  numbers: {
    // Signed percentage change. {n} is the absolute value, already rounded.
    percentUp: "+{n}%",
    percentDown: "\u2212{n}%",
    percentFlat: "{n}%",
  ...
  /* ------------------------------------------------------------------ */
  /* Bus lanes page (/buslanes)                                          */
  /* ------------------------------------------------------------------ */
  busLanes: {
    kicker: "AC Transit, 2019\u20132026",
    title: "Bus lanes and bus speeds",
    lede:
      "Four streets in the AC Transit service area gained bus lanes with a documented start date "
      + "between 2019 and 2026. This page compares how fast buses moved on each in the year before "
      + "and the year after.",
    linkExplore: "Explore the map",
    linkStory: "Read the story",
    linkMethodology: "Methodology",
    // {message} is the underlying fetch error.
    error: "The data could not be loaded: {message}",
    loading: "Loading\u2026",

    headingBeforeAfter: "Speed before and after",
    headingMonthly: "Month by month",
    headingLanes: "The lanes",
    headingMethod: "How this was measured",

    legendBefore: "12 months before the lane",
    legendAfter: "12 months after",
    legendLane: "Bus lane street",
    legendNetwork: "All AC Transit streets",
    legendBands: "Before and after windows",

    dumbbellAria: "Average bus speed on each bus lane before and after it was added",
    colChange: "Change",
    colNetwork: "Network",
    // Row subtitle under a lane name.
    laneSubtitle: "{city} \u00b7 {month}",
    dumbbellCaption:
      "Average bus speed in miles per hour, all day, including time at stops and lights. "
      + "\u201cNetwork\u201d is the change across every AC Transit street over the same months.",

    // {name} is the street.
    laneChartAria: "Monthly average bus speed on {name} and across the network, 2019 to 2026",
    laneAdded: "Lane added",
    ...
    colAfter: "After",
    sources: "Sources:",
    // Shown when the after-window is shorter than the full comparison window.
    partialMonths: " ({n} months)",
    // <0> is the street name in bold.
    laneRowName: "<0>{name}</0>, {city}",

    headingUndated: "Other bus lanes without a start date",
    undatedNote:
      "These are mapped as bus lanes in OpenStreetMap, but no source gave a date they were added, "
      + "so they are not compared.",
    // <0> is the street name in bold. {km} is already fixed to 2 decimals.
    undatedItem: "<0>{name}</0>, {city} ({km} km mapped). {note}",

    method1:
      "Speeds come from AC Transit\u2019s automatic passenger counters. For each trip, the time "
      + "between the doors opening at one stop and the next is divided into the distance between "
      + "them, so the figures include time spent at stops and traffic lights. They are averaged by "
      + "month over every trip on each street, weighted by how much service runs at each time of "
      + "day.",
    method2:
      "A street segment counts as part of a lane when most of it lies within 15 meters of the lane "
      + "as mapped in OpenStreetMap. \u201cBefore\u201d is the 12 months ending the month before "
      + "the lane was added and \u201cafter\u201d the 12 months starting the month after; the "
      + "month itself is left out.",
    method3:
      "A before-and-after comparison cannot separate a lane from everything else that changed at "
      + "the same time. Traffic fell sharply across the region in 2020, when the International "
      + "Blvd. and Broadway lanes opened; the 1T replaced the local route 1 with fewer stops the "
      + "same day Tempo opened; and AC Transit redesigned its routes in August 2025, around the "
      + "Durant Ave. and Bancroft Way projects. The network column shows how speeds changed "
      + "everywhere over the same months, for comparison.",
    method4:
      "The street map these speeds are measured on was updated in January 2022 and April 2025. A "
      + "street\u2019s speed can shift a little at those dates because its segments are drawn "
      + "differently, not because buses changed speed. The Durant Ave. and Bancroft Way "
      + "comparisons span the April 2025 update, so treat their small changes with extra caution.",
  },
  ...
  storyMap: {
    ...
    commuteNote: "Average weekday, 5–9 a.m., {direction} UC Berkeley.",
    ...
    // City comparison
    citiesTitleYear: "By city: {from}–{to} vs. the year before the pandemic",
    citiesTitleWeek: "By city: four weeks from this date vs. February 2020",
  ...
  diagrams: {
    ...
    odAria: "Boardings and alightings along a route, and the fitted origin-destination matrix",
    odCaption:
      "Left: one route-direction's average morning profile — boardings above the line, "
      + "alightings below. Right: the flow matrix fitted to it. Only the upper triangle can be "
      + "filled, because a rider cannot get off before they get on; the row sums are the boardings "
      + "and the column sums the alightings. The matrix shown was produced by running the fit in "
  ...
  explorer: {
    ...
    hintCommuteFocus:
      "Morning (5-9am) inferred riders {direction} <0>{label}</0>, {period} snapshot. Dot size and "
